# 實體機器路線圖（硬體 / 韌體）

> 狀態：**階段 1 基本完成**（組裝＋接線＋L298N 通電測試 OK，車會走了 🎉 2026-06-10）
> 建立日期：2026-06-03
> 延伸自 [vision-yolo.md](vision-yolo.md) 的討論

## 重大決定：要做「真的會走的機器」

GreenKeeper 不只停在模擬器，目標是做出**一台真實、會自己走的車**（割草刀片是最後最後的事，前期先不裝）。

模擬器不會白做——它是這台機器的「**大腦練習場**」。

## 三層架構

一台自走機器拆成三層：

| 層 | 比喻 | 做什麼 | 例子 |
|---|---|---|---|
| ① 感知 | 眼睛 | 看前面有什麼 | 鏡頭拍照 → YOLO 判斷「草 / 障礙物」 |
| ② 決策 | 大腦 | 決定怎麼走 | 「左邊還沒割，往左轉」 |
| ③ 動作 | 手腳 | 把決定變成動作 | 「左輪慢、右輪快」→ 車子左轉 |

## 模擬器目前的定位

模擬器**只做了第 ② 層（大腦），而且是「上帝視角作弊版」**：

- **沒有真的輪子**：每一格時間把圓點「瞬移」一小段（`sim.js` 的 `this.x += cos(angle)*speed`），沒有馬達 / 轉速。
- **沒有真的眼睛**：演算法直接讀整張地圖陣列，等於早就知道答案，所以不需要 YOLO。

真機要做的事 = 把大腦左右兩端的「假器官」換成真的：

```
        ┌─────────── 真機 ───────────┐
眼睛 → │  鏡頭+YOLO  →  大腦  →  馬達  │ → 輪子
        └────────────────────────────┘
模擬器 │   (假sensor)   大腦  (假瞬移)  │
        └────────────────────────────┘
            ↑換成真的          ↑換成真的
```

中間的大腦在模擬器裡先調好（這也是 vision-yolo.md 的路線 B），真機只是換掉兩端。

## 真機需要的東西

### 🦴 身體（會走的車）
- 底盤 + 兩個輪子各一顆馬達 → 兩輪轉速不同就會轉彎（差速轉向，坦克原理）
- 馬達驅動板（motor driver）→ 馬達吃電大，主機板小電流推不動，需要它當中間放大器
- 電池

### 👁 眼睛
- 一顆 USB 鏡頭（視訊鏡頭等級即可）

### 🧠 兩個腦（新手最容易卡的觀念）
真機通常**兩個處理器分工**：

| | 誰 | 負責 | 特性 |
|---|---|---|---|
| 大腦 | 樹莓派 / Jetson | 跑 YOLO、做決策 | 算力大但反應慢 |
| 小腦 | Arduino / ESP32 | 即時控制輪速、讀避障感測器 | 反應快但笨 |

**為什麼分兩個**：看影像耗算力會卡頓，但控制輪子必須毫秒級即時不能等。所以讓快但笨的小腦顧輪子，慢但聰明的大腦看路，兩者用一條序列線溝通。

> ⚠️ 割草刀片很危險。第一階段只做「會自己走的車」，**不裝刀片**。先解決導航。

## 分階段路線圖（每階段都有可動成果）

1. **遙控車** — 車能動、用鍵盤/手機能開。先搞定輪子、馬達、馬達驅動板。（純硬體，還沒 AI）
2. **眼睛接上** — 鏡頭畫面進電腦，YOLO 能框出「草 / 障礙物」。（單獨玩 YOLO，先不上車）
3. **接大腦** — 把模擬器調好的決策演算法，改成吃「YOLO 看到的東西」，自動開車。
4. **（很久以後）** 裝刀片、防水、電量 / 回充座。

## 階段 2 的效能現實：Pi 4 純 CPU 跑得動 YOLO 嗎？

> 來源：視覺辨識可行性技術備忘（2026-06-04）。結論：**fps 不是問題，安全架構才是重點。**

除草機移動很慢（約 0.3 m/s），對 fps 的需求遠低於一般即時影像應用：

| 推論 fps | 每次偵測間機器前進距離 |
|----------|------------------------|
| 5 fps    | 約 6 公分 |
| 3 fps    | 約 10 公分 |

**Pi 4 純 CPU（YOLOv8n + NCNN + `imgsz=320`）可達約 5~8 fps，足以支援慢速除草機的避障反應。** 不需要先衝去買加速器。

### 效能優化重點（純 CPU 即可達標）

1. 只用 **YOLOv8n**（nano，唯一適合 Pi 4 的尺寸），匯出 **NCNN** 格式（`model.export(format="ncnn")`）。
2. 降低輸入解析度 `imgsz=320`。
3. **多執行緒分離「影像擷取」與「推論」**，只處理最新影格、丟棄舊影格，避免延遲累積。
4. 視需要加入跳幀（每 2~3 幀偵測一次）。

### 若需更高效能 / 更省心

- 加裝 **Coral USB Accelerator（Edge TPU）** 可達 30+ fps。
- 或升級 **Pi 5**（CPU 約快 2~3 倍）。
- 兩者都是「之後嫌慢再考慮」的選項，階段 2/3 先用手上的 Pi 4 即可。

### 即時偵測腳本（Pi 4 純 CPU，多執行緒）

> 使用前先 `model.export(format="ncnn")` 產生 NCNN 模型。階段 2 可直接拿來跑。

```python
import cv2, threading, time
from ultralytics import YOLO

model = YOLO("yolov8n_ncnn_model")

class Camera:
    """背景執行緒持續抓最新影格，只保留最新一張"""
    def __init__(self, src=0):
        self.cap = cv2.VideoCapture(src)
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        self.frame = None
        self.running = True
        threading.Thread(target=self.update, daemon=True).start()
    def update(self):
        while self.running:
            ok, f = self.cap.read()
            if ok:
                self.frame = f          # 舊影格直接覆蓋
    def read(self):
        return self.frame
    def stop(self):
        self.running = False
        self.cap.release()

cam = Camera(0)
time.sleep(1)                            # 相機暖機
prev = time.time()
while True:
    frame = cam.read()
    if frame is None:
        continue
    results = model(frame, imgsz=320, verbose=False)   # 320px 加速
    annotated = results[0].plot()
    now = time.time()
    fps = 1 / (now - prev)
    prev = now
    cv2.putText(annotated, f"FPS: {fps:.1f}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.imshow("YOLOv8n - Pi4", annotated)
    if cv2.waitKey(1) & 0xFF == ord("q"):
        break
cam.stop()
cv2.destroyAllWindows()
```

### YOLO 在本專案的角色（補充）

- COCO 預訓練的 YOLOv8n 已可偵測 `person` / `dog` / `cat` → 直接用於避開人與寵物。
- 「草地 vs 花圃 vs 步道」「邊界」COCO 不涵蓋 → 需自行拍攝資料集**微調（fine-tune）**（呼應 vision-yolo.md 路線 A）。
- 進階：寵物便便偵測繞開（現行商用割草機已有此功能）。

## Pi ↔ Uno 怎麼合體（階段 3）

三層各就各位，這台車（Uno 套件）= 手腳，Pi + webcam = 眼睛 + 大腦：

```
👁 webcam ──► 🧠 Pi 4 ──(指令)──► 🦿 Uno + L298N ──► 輪子
  (眼睛)      (大腦,跑YOLO)        (自走車套件 = 手腳)
```

**怎麼溝通：一條 USB 線走「序列埠 (Serial)」**（最簡單、最穩、重用套件的 USB 線）：

- Pi 的 USB ↔ Uno 的 USB 接起來，就像一根「傳紙條的管子」。約定 baud rate（如 9600）。
- Pi（Python）寫指令：`ser.write(b'a')` → Uno（`Serial.read()`）讀到 `'a'` → 左轉。
- **跟套件的藍牙範例幾乎一樣**，只是把 `BT.read()` 換成 `Serial.read()`，馬達邏輯不動。
- 雙向：Uno 也能把超音波距離寫回管子給 Pi 參考。

指令對照（沿用範例）：`w` 前進／`x` 後退／`a` 左轉／`d` 右轉／`s` 停。

> 其他方式（先別用）：GPIO TX/RX 直連需處理 **3.3V(Pi) vs 5V(Uno) 電壓轉換**，否則燒 Pi；藍牙可無線但 Pi 配對較麻煩。**USB 序列埠最省事。**

⚠️ **供電分開**：USB 線只走資料。**電池盒推 Uno + 馬達；Pi 用自己的行動電源。** 否則馬達一啟動電壓掉，Pi 會當機重開。

## 真車演算法進化路線（模擬器 = 預言書）

`sim.js` 的 ALGORITHMS 清單就是真車的升級路線圖，照同樣順序爬：

| 真車進化 | 對應模擬器演算法 | 需要什麼 |
|---|---|---|
| 1. 反應式（現在） | 隨機碰撞反彈 | 只要眼前判斷，不用地圖 ← `auto_drive.py` 在這層 |
| 2. 沿邊走 | 沿牆走 | 超音波 |
| 3. 知道走了多遠 | — | 輪子編碼器 / IMU（里程計） |
| 4. 邊走邊畫地圖 | 建圖式掃描（SLAM） | 定位 + 記錄 |
| 5. 分區規劃（終局） | 梯形分解（BCD）🥇 | 地圖 + 定位都有了才能上 |

### 繞障礙的分工（YOLO 看不見牆！）

COCO 80 類**沒有「牆壁」「坑洞」**，所以避障必須互補：

- **YOLO**：認得的東西（人、寵物、家具、盆栽）→ 看到在畫面哪半邊就往反向繞
- **超音波**：認不得但擋路的東西（牆、任何近距離物體）→ 不管是什麼，太近就停/轉
- **坑洞**：最難。要嘛自己訓練（vision-yolo.md 路線 A），要嘛靠碰撞/抬升感測（階段 4 安全層）

## 階段 4 的安全架構（裝刀片前必讀）

> 來源：視覺辨識可行性技術備忘（2026-06-04）。前期不裝刀片，但**裝之前一定要先把這層想清楚**。

除草機有**旋轉刀片**，偵測人/寵物/手是攸關安全的事。
3~5 fps 的視覺加上推論延遲與煞停時間，反應太慢——**視覺絕不能當刀片安全的最後防線。**

正規做法是「**冗餘感測 + 硬體即時切斷**」，視覺僅為輔助層：

| 層級 | 採用方案 | 角色 |
|------|----------|------|
| 邊界界定 | 周界埋線 或 RTK-GPS | 決定可割範圍，**不使用視覺** |
| **硬體安全（最高優先）** | 抬起感測 + 傾斜感測 + 碰撞感測 | 觸發時**直接硬體切斷刀片電源**，不經過 Pi 運算 |
| 近距離避障 | 超音波 / ToF 測距（如 VL53L0X） | 便宜、可靠、不受光線影響 |
| 智慧視覺 | Pi 4 + webcam + YOLOv8n | 辨識並繞開人、寵物、玩具、花圃 |

**核心原則：刀片安全交由硬體感測器即時切斷；視覺只負責更聰明的導航與繞障。**

### 待確認（階段 4 之前）

- [ ] 邊界方案決定：周界埋線 vs RTK-GPS
- [ ] 硬體安全感測器選型與切斷電路設計（抬起／傾斜／碰撞）
- [ ] 近距離避障感測器選型（超音波 / ToF，如 VL53L0X）
- [ ] 電源預算評估（Pi 4 / Pi 5 / +Coral 的耗電與電池續航）

## 心理準備
- 時間：好幾個月。
- 花費：約台幣 3,000～10,000。
- 動手：需要接線（可能要一點焊接）。
- 策略：分階段做，每階段都有會動的成果，避免卡死。

## 下一步（待辦）

硬體已大致到位（Pi 4 大腦、Uno 套件手腳、webcam 眼睛、超音波避障）。詳見 [hardware-inventory.md](hardware-inventory.md)。

### 調整後的優先順序（2026-06-10）

階段 1 車已會走。接下來改成：

1. **Pi 整合（視覺）** ← 最優先。含三塊：
   - **動作鏈** ✅（**2026-06-11 打通！Pi 跑 `robot/drive_test.py` 車會動**）：
     Pi → USB 序列埠 → Uno（`firmware/serial_control`）→ 馬達。
     Uno 端 1 秒無指令自動停車；Pi 端 scp 部署（Pi 的 git clone 卡 SSL，
     修法：`sudo apt-get install --reinstall ca-certificates`，待驗證）
   - **視覺** ✅（**2026-06-11 同日打通！`robot/vision_test.py` 框出 tv/keyboard**）：
     ultralytics 裝在 Pi（piwheels 快）；公司網路 FortiGate 攔 github SSL，
     權重/程式一律 **scp 部署**（PC 下載 `yolov8n.pt` → scp 到 Pi 家目錄）。
     有看到誤判（電話認成 keyboard 35%）＝ vision-yolo.md 說的偵測雜訊，真實上演
   - **接合** ✅（`robot/auto_drive.py`：看到 person 停、否則前進，--drive 實測會動）
   - **開機自動啟動** ✅（2026-06-12 `greenkeeper.service` 部署＋重開機驗收通過；
     馬達電池開關 = 武裝開關，上電即服役）
   - **下一步：地圖機制**（編碼盤里程計 或 航位推算）→ 通往 SLAM / 梯形分解
   - 觀念：CNN/YOLO 負責「看」，方向由**自己的規則邏輯**決定（非端到端讓 CNN 直接控向）
2. **超音波測距（保留，可延後）**：避障安全層（便宜可靠，自動駕駛時當防撞底線）。跟視覺不重複——它是毫秒級、不受光線影響的即時反射；可等 Pi 跑通後再補裝。
3. ~~**藍牙模組**~~ **→ 已決定跳過**（2026-06-10）：原為手機遙控/測試，但 USB 序列埠能測一樣的馬達協定，藍牙等於多繞一圈。詳見 [car-assembly.md](car-assembly.md) 決策紀錄。

> 左右輪速不對稱：用 `LEFT_SPEED`/`RIGHT_SPEED` 微調即可；走直是 IMU/微調的事，不是視覺的工作。

- [x] 決定大腦 → **樹莓派 Pi 4**
- [x] 決定手腳 → **傑森創工 Uno 2WD 套件**（這台用 Uno，Mega 備用）
- [ ] 確認電池（18650/AA）→ 下單套件
- [x] **階段 1**：組裝 ✅ / Blink ✅ / L298N 馬達會動 ✅（**車會走了！** 2026-06-10）→ 剩：左右輪速微調（TT 馬達兩側有差，用 `LEFT_SPEED`/`RIGHT_SPEED` 修）。避障（超音波）延後、藍牙跳過。
- [ ] **階段 2**：筆電 + webcam 跑 YOLO（`pip install ultralytics`，用 `yolov8n`），框出物體 → 上 Pi 4 用 NCNN + `imgsz=320` 跑多執行緒腳本（見「階段 2 的效能現實」，純 CPU 約 5~8 fps）
- [ ] **階段 3**：Pi 裝 webcam 上車，USB 序列埠接 Uno，把決策接上（見上方「Pi ↔ Uno 怎麼合體」）
